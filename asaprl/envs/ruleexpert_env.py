...
from asaprl.utils.env_utils.agent_ruleexpert_manager_utils import RuleExpertAgentManagerRoundabout, RuleExpertAgentManagerIntersection, RuleExpertAgentManagerHighway
from asaprl.utils.env_utils.ruleexpert_engine_utils import initialize_engine, close_engine, \
    engine_initialized, set_global_random_seed, MacroRuleExpertEngine
...
class MetaDriveRuleExpertEnv(BaseEnv):
    ...
    def __init__(self, config: dict = None):
        ...
        if "O" in self.config['map']:
            self.agent_manager = RuleExpertAgentManagerRoundabout(
                init_observations=self._get_observations(), init_action_space=self._get_action_space()
            )
        elif "X" in self.config['map']:
            self.agent_manager = RuleExpertAgentManagerIntersection(
                init_observations=self._get_observations(), init_action_space=self._get_action_space()
            )
        else:
            self.agent_manager = RuleExpertAgentManagerHighway(
                init_observations=self._get_observations(), init_action_space=self._get_action_space()
            )
        self.action_type = DiscreteMetaAction()   # does not quite matter since the position of vehicle is set directly

        # lazy initialization, create the main vehicle in the lazy_init() func
        self.engine: Optional[MacroRuleExpertEngine] = None
        ...
        self.vehicle_last_position = (0, 0)
        self.vehicle_last_position_initialized = False
    ...
    def _step_macro_simulator(self, actions):
        ...
        obs_state_one_skill = {}
        vehicle_current_pos_one_skill = {}
        for v_id, v in self.vehicles.items():
            o = self.observations[v_id].observe(v)
            obs_state_one_skill[v_id] = {'obs_one_skill':[o], 'abs_state_one_skill':[np.array(v.position)], 'relative_state_one_skill': np.zeros((1,2)), "vehicle_start_speed": v.last_spd}
            vehicle_current_pos_one_skill[v_id] = v.position
        for frame in range(frames):
            ...
            for v_id, v in self.vehicles.items():
                ...
                if frame == 0: 
                    obs_state_one_skill[v_id]['relative_state_one_skill'] = np.expand_dims(np.array(v.position) - np.array(vehicle_current_pos_one_skill[v_id]), 0)
                else:
                    obs_state_one_skill[v_id]['relative_state_one_skill'] = np.vstack((obs_state_one_skill[v_id]['relative_state_one_skill'], np.expand_dims(np.array(v.position) - np.array(vehicle_current_pos_one_skill[v_id]), 0)))
    ...
    def _post_process_config(self, config): 
        config = super(MetaDriveRuleExpertEnv, self)._post_process_config(config)
    ...
    def reward_function(self, vehicle_id: str): 
        ...
        if not self.vehicle_last_position_initialized:
            self.vehicle_last_position_initialized = True
            self.vehicle_last_position = vehicle.position
        long_last, _ = current_lane.local_coordinates(self.vehicle_last_position)
        self.vehicle_last_position = vehicle.position
    ...
    def setup_engine(self):
        super(MetaDriveRuleExpertEnv, self).setup_engine()
...
register(
    id='RuleExpert-v1',
    entry_point='asaprl.envs:MetaDriveRuleExpertEnv',
)
